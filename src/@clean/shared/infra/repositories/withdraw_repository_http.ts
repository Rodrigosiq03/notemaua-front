...
export class WithdrawRepositoryHttp implements IWithdrawRepository {
  async getAllNotebooks(idToken: string): Promise<[Notebook, Withdraw[]][]> {
    var notebooks: [Notebook, Withdraw[]][] = [
      [
        new Notebook({ numSerie: '34000', isActive: false }),
        [
          new Withdraw({
            numSerie: '34000',
            email: 'erro@maua.br',
    ...
    try {
      ...
      const { data, status } = await axios.get<[Notebook, Withdraw[]][]>(url, {
  ...
  async finishWithdraw(numSerie: string, idToken: string): Promise<Withdraw> {
    ...
    try {
      ...
      const { data, status } = await axios.put<Withdraw>(
        url,
        { num_serie: numSerie },
        { headers: { Authorization: `Bearer ${idToken}'` } }
      );

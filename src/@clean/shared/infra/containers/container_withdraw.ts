...
containerWithdraw.bind(RegistryWithdraw.GetAllWithdrawsUsecase).toDynamicValue((context) => {
  if (process.env.NEXT_PUBLIC_STAGE === "TEST") {
    return new GetAllWithdrawsUsecase(context.container.get(RegistryWithdraw.WithdrawRepositoryMock));
  } else if (process.env.NEXT_PUBLIC_STAGE === "DEV") {
    return new GetAllWithdrawsUsecase(context.container.get(RegistryWithdraw.WithdrawRepositoryHttp));
  }
  else {
    return new GetAllWithdrawsUsecase(context.container.get(RegistryWithdraw.WithdrawRepositoryMock));
  }
...
containerWithdraw.bind(RegistryWithdraw.FinishWithdrawUsecase).toDynamicValue((context) => {
  if (process.env.NEXT_PUBLIC_STAGE === "TEST") {
    return new FinishWithdrawUsecase(context.container.get(RegistryWithdraw.WithdrawRepositoryMock));
  } else if (process.env.NEXT_PUBLIC_STAGE === "DEV") {
    return new FinishWithdrawUsecase(context.container.get(RegistryWithdraw.WithdrawRepositoryHttp));
  }
  else {
    return new FinishWithdrawUsecase(context.container.get(RegistryWithdraw.WithdrawRepositoryMock));
  }

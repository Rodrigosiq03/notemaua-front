...
import { RegistryWithdraw, containerWithdraw } from '../@clean/shared/infra/containers/container_withdraw';
...
export function WithdrawProvider({ children }: PropsWithChildren) {
    ...
    async function createWithdraw(numSerie: string, email: string) {
        try {
            const withdrawCreated = await createWithdrawUsecase.execute(numSerie, email);
